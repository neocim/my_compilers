...
#[proc_macro_derive(IntoDiag, attributes(diagnostic))]
pub fn into_diag_derive(input: TokenStream) -> TokenStream {
    ...
    let implementation = quote! {
        Diagnostic::new(
            emitter,
            DiagMessages::new(vec![DiagMessage::new(
    ...
    let gen = quote! {
        impl<'a, E: Emitter> IntoDiag<'a, E> for #name {
            fn into_diag(&self, emitter: &'a E) -> Diagnostic<'a, E> {
                #implementation
            }
        }
    };
...
// an error saying that it should only be used with structures with named fields
